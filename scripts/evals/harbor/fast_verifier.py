...
只在「脚本依赖完全落在预置集合内」时才改写：一旦 test.sh 还需要 apt 系统包或
额外的 uvx 依赖，就原样放行。宁可慢一点联网，也不能把任务自己的依赖静默丢掉。
"""
...
def _fast_eligible(lines: list[str]) -> bool:
    ...
    for line in lines:
        if _APT_INSTALL.match(line):
            ...
            if packages - _IGNORABLE_APT_PACKAGES:
                return False
...
def _patch_test_script(path: Path) -> None:
    ...
    if eligible:
        ...
        output.extend(
            [
                "# Biny fast verifier: 判分依赖已预置，跳过联网引导",
        ...
        # 依赖超出预置范围，下面的 uvx 仍要联网解析。作业级默认是 UV_OFFLINE=true，
        # 这里显式关掉，否则离线模式会把正常的在线解析也一起挡掉。
        output.extend(
            [
                "# Biny fast verifier: 任务自带额外依赖，改回联网解析",
    ...
    while index < len(lines):
        ...
        # uv 已经随判分包预置，这条在线安装器无论走哪条路径都没必要跑。
        if _UV_INSTALLER.match(line):
            output.append("# Biny fast verifier: uv already preloaded")
            _, index = _consume_logical(lines, index)
            continue

        if eligible and (_APT_UPDATE.match(line) or _APT_INSTALL.match(line)):
            output.append("# Biny fast verifier: system deps already available")
            _, index = _consume_logical(lines, index)
            continue

        if eligible and _PIP_INSTALL.match(line):
            output.append("# Biny fast verifier: pytest and ctrf already preloaded")
            _, index = _consume_logical(lines, index)
            continue
        ...
        if eligible and _UVX_START.match(line):
            ...
            if pytest_line is not None:
                ...
                output.append(f"if [ -x {_PYTEST_BIN} ]; then")
