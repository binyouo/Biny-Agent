...
Sleep 默认每天本地时间 03:00 调度，处理重复、临时记忆过期、相似聚类及可选 LLM 合成。默认临时 TTL 和归档保留期均为 30 天。预览不改事实；取消停止后续工作，不回滚已提交结果。启动时处理上次中断的运行，同日失败有退避限制。自动任务要求 Runtime Host 存活，不等于系统级唤醒。
